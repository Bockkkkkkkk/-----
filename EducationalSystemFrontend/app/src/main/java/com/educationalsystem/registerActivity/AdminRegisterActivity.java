...
import com.educationalsystem.common.Admin;
...
public class AdminRegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        //将admin发送给后端，等待后端返回结果
        admin.setAccount(account_et.getText().toString());
        admin.setPassword(password_et.getText().toString());
        String response;
        try {
            response = netUtil.doPost("/admin/register", JSON.toJSON(admin).toString());
